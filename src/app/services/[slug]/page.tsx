...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  return (
    <div className="flex flex-col">
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {service.whatWeProvide.map((item, idx) => (
              <div
                key={idx}
                className="p-4 rounded-xl border border-slate-200 dark:border-white/10 flex items-start gap-3 bg-slate-50/50 dark:bg-white/5"
              >
                <div className="w-5 h-5 rounded-full bg-emerald-100 dark:bg-emerald-500/10 text-[#20A779] flex items-center justify-center shrink-0 mt-0.5">
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-white/10 space-y-2">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500">Core Technologies</h4>
            <div className="flex flex-wrap gap-2">
              {service.technologies.map((t, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1 rounded-md text-xs font-mono bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300"
                >
                  {t}
                </span>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {service.typicalUseCases.map((uc, idx) => (
                <div key={idx} className="clean-card p-6 rounded-xl space-y-2">
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white">{uc.title}</h4>
        ...
        <section className="surface-white py-16 px-4 sm:px-6 lg:px-8 border-b border-slate-200 dark:border-white/10">
          <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold tracking-tight">Related Work</h3>
              ...
              {relatedProjects.map((proj) => (
                ...
                >
                  ...
                  <div className="p-4 space-y-1">
                    <h4 className="text-sm font-bold group-hover:text-[#3B82F6] transition-colors">{proj.title}</h4>
                    <p className="text-xs text-slate-500 line-clamp-1">{proj.shortDescription}</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="surface-warm py-16 px-4 sm:px-6 lg:px-8 border-b border-slate-200 dark:border-white/10">
        <div className="max-w-4xl mx-auto space-y-6">
          <h3 className="text-lg font-bold tracking-tight">How we deliver this service.</h3>
          <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
            {service.process.map((p, idx) => (
              <div key={idx} className="p-3.5 rounded-lg bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 space-y-1">
                <span className="text-[10px] font-mono font-bold text-[#C7A45D]">{p.step}</span>
                <h5 className="text-xs font-bold">{p.title}</h5>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-3 leading-relaxed">{p.description}</p>
              </div>
            ))}
          </div>
        </div>
